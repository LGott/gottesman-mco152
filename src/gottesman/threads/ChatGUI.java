 ...
 * port that is specified by the user. It can request a connection to another ChatGUI window on a specified computer and
 * port. The window has an input box where the user can enter messages to be sent over the connection. A connection can
 * be closed by clicking a button in the window or by closing the window. It is possible to open additional windows to
 * support simultaneous chats (or to test the program by opening a connection from one window to another). This class
 * contains a main() routine, so it can be run as a stand-alone application.
 */
public class ChatGUI extends JFrame {
	...
	/**
	 * Used to keep track of where on the screen the previous window was opened, so that the next window can be placed
	 * at a different position.
	 */
	private static Point previousWindowLocation;

	/**
	 * The number of windows that are currently open. If this drops to zero, then the program is terminated by calling
	 * System.exit();
	 */
	private static int openWindowCount;

	/**
	 * The number of windows that have been created. This is used in the title bar of the second and subsequent windows.
	 */
	...
	/**
	 * The main() routine makes it possible to run this class as an application; it just creates a ChatGUI window and
	 * makes it visible.
	 */
	 ...
	 */
	private JButton newButton, listenButton, connectButton, closeButton, clearButton, quitButton, saveButton,
			sendButton;
	...
	/**
	 * Contains a transcript of messages sent and received, along with information about the progress and state of the
	 * connection.
	 */
	private JTextArea transcript;

	/**
	 * Constructor creates a window with a default title. The constructor does not make the window visible.
	 */
	public ChatGUI() {
		this(windowsCreated == 0 ? "Chat Window" : "Chat Window #" + (windowsCreated + 1));
	}

	/**
	 * Constructor creates a window with a specified title. The constructor does not make the window visible.
	 */
	public ChatGUI(String title) {
		...
		if (previousWindowLocation == null) {
			previousWindowLocation = new Point((int) (40 + (30 * Math.random())), (int) (80 + (50 * Math.random())));
		} else {
	...
	/**
	 * Defines responses to buttons, and when the user presses return in the message input box.
	 */
			...
			if (source == newButton) {
				...
				if ((connection == null) || (connection.getConnectionState() == ConnectionState.CLOSED)) {
					...
					try {
						...
						JOptionPane.showMessageDialog(ChatGUI.this, portString + "is not a legal port number.");
						return;
					}
	 ...
	 * 
	 * @param message
	 *            text to be added; a line feed is added at the end.
	 */
	...
	private class ConnectionHandler extends Thread {
		...
		/**
		 * Open a connection to specified computer and port. The constructor does not perform any network operations; it
		 ...
		 * an end-of-stream is encountered on the input stream.) It posts a message to the transcript and sets the
